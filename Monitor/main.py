...
from utils import user_login, schedule_data_updates, log_notification, user_notification
from config import NETWORK_PATH_PREFIX, USER_NAMES, get_user
...
class BetViewerApp:
    ...
    def initialize_ui(self):
        ...
        self.root.tk.call('source', resource_path('Forest-ttk-theme-master/forest-light.tcl'))
    ...
    def import_logo(self):
        logo_image = Image.open(resource_path('splash.ico'))
        logo_image.thumbnail((70, 70))
        self.company_logo = ImageTk.PhotoImage(logo_image)
        self.root.iconbitmap(resource_path('splash.ico'))
    ...
    def start_background_tasks(self):
        ...
        threading.Thread(target=self.database_manager.periodic_cache_update, daemon=True).start()  # Start the periodic cache update
    ...
    def apply_bonus_points(self):
        ...
        def submit_bonus():
            ...
            log_notification(f"{selected_user} received a bonus of {points:.2f} points from {get_user()}", True)
        ...
        bonus_window.iconbitmap(resource_path('splash.ico'))
