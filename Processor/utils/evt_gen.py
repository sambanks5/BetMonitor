...
class EventGenerator:
    ...
    def _fetch_leagues_data(self, sport_id):
        try:
            response = requests.get(f"http://localhost:4000/GetUniqLeagueListFromEventList?SportId={sport_id}")
            if response.status_code == 200:
    ...
    def generate_event(self):
        ...
        if selected_sport and selected_league:
            ...
            try:
                response = requests.get(f"http://localhost:4000/GenerateFileByLeagueId?SportId={sport_id}&LeagueId={league_id}")
                if response.status_code == 200:
